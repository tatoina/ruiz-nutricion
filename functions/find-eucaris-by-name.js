...
async function findEucarisByName() {
  try {
    console.log('Buscando por nombre y apellido...\n');
    ...
    for (const doc of usersSnapshot.docs) {
      ...
      const surname = (data.surname || data.apellidos || '').toLowerCase();
      
      if (name.includes('eucaris') || surname.includes('martinez') || surname.includes('martínez')) {
        found = true;
        console.log('═══════════════════════════════════════');
        ...
        console.log(`DOB: ${data.dob || data.birthDate || 'N/A'}`);
...
`;
    ...
    if (!found) {
      console.log('❌ No se encontró usuario con ese nombre o apellido');
      console.log('\nListando todos los nombres y apellidos:');
      usersSnapshot.forEach(doc => {
        const data = doc.data();
        console.log(`- ${data.name || data.nombre || 'SIN NOMBRE'} ${data.surname || data.apellidos || ''}`);
      });
    }
